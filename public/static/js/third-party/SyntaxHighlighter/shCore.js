...
(function (undefined) {
  ...
  XRegExp = function (pattern, flags) {
    var output = []
    var currScope = XRegExp.OUTSIDE_CLASS
    var pos = 0
    var context; var tokenResult; var match; var chr; var regex
    ...
    while (pos < pattern.length) {
      ...
      if (tokenResult) {
        ...
        if (match = nativ.exec.call(nativeTokens[currScope], pattern.slice(pos))) {
          ...
          if (chr === '[') { currScope = XRegExp.INSIDE_CLASS } else if (chr === ']') { currScope = XRegExp.OUTSIDE_CLASS }
  ...
  var flagClip = /[^gimy]+|([\s\S])(?=[\s\S]*\1)/g // Nonnative and duplicate flags
  var quantifier = /^(?:[?*+]|{\d+(?:,\d*)?})\??/
  ...
  var compliantExecNpcg = nativ.exec.call(/()??/, '')[1] === undefined // check `exec` handling of nonparticipating capturing groups
  ...
  String.prototype.replace = function (search, replacement) {
    ...
    var captureNames; var result; var str; var origLastIndex
...
if (typeof (SyntaxHighlighter) === 'undefined') {
  var SyntaxHighlighter = (function () {
      ...
      /**
        ...
        handler: function (e) {
          ...
          function getValue (name) {
            ...
            var match = r.exec(className)

            return match ? match[1] : null
          };

          var highlighter = getHighlighterById(findParentElement(target, '.syntaxhighlighter').id)
          var commandName = getValue('command')
        ...
        // Ordered lis of items in the toolbar. Can't expect `for (var n in items)` to be consistent.
          ...
          expandSource: {
            getHtml: function (highlighter) {
              ...
              return sh.toolbar.getButtonHtml(highlighter, 'expandSource', title || sh.config.strings.expandSource)
            },
    ...
    function findElement (target, search, reverse /* optional */) {
      ...
      var propertyToFind	= { '#': 'id', '.': 'className' }[search.substr(0, 1)] || 'nodeName'
      var expectedValue
      var found
    ...
    function parseParams (str) {
      var match
      var result = {}
      var arrayRegex = new XRegExp('^\\[(?<values>(.*?))\\]$')
    ...
    function getMatches (code, regexInfo) {
      ...
      var match = null
      var matches = []
      var func = regexInfo.func ? regexInfo.func : defaultAdd
    ...
    function quickCodeHandler (e) {
      ...
      var container = findParentElement(target, '.container')
    ...
    sh.HtmlScript = function (scriptBrushName) {
      ...
      var bracketsRegex = null
      var ref = this
      var methodsToExpose = 'getDiv getHtml init'.split(' ')
      ...
      function process (match, info) {
        var code = match.code
        var matches = []
        var regexList = scriptBrush.regexList
        var offset = match.index + match.left.length
        var htmlScript = scriptBrush.htmlScript
        var result
    ...
    /**
      ...
      figureOutLineNumbers: function (code) {
        var lines = []
        var firstLine = parseInt(this.getParam('first-line'))
      ...
      getLineNumbersHtml: function (code, lineNumbers) {
        var html = ''
        var count = splitLines(code).length
        var firstLine = parseInt(this.getParam('first-line'))
        var pad = this.getParam('pad-line-numbers')

        if (pad == true) { pad = (firstLine + count - 1).toString().length } else if (isNaN(pad) == true) { pad = 0 }

        for (var i = 0; i < count; i++) {
          var lineNumber = lineNumbers ? lineNumbers[i] : firstLine + i
          var code = lineNumber == 0 ? sh.config.space : padNumber(lineNumber, pad)
      ...
      getCodeLinesHtml: function (html, lineNumbers) {
        ...
        var padLength = this.getParam('pad-line-numbers')
        var firstLine = parseInt(this.getParam('first-line'))
        var html = ''
        var brushName = this.getParam('brush')
