...
(function () {
  ...
  var drawStep = [] // undo redo存储
  var drawStepIndex = 0 // undo redo指针
  ...
  scrawl.prototype = {
    ...
    _addBoardListener: function (saveNum) {
      ...
      var isMouseUp = false
      var buttonPress = 0; var button; var flag = ''
    ...
    _addClearSelectionListenter: function () {
      ...
      domUtils.on(doc, 'mousemove', function (e) {
        if (browser.ie && browser.version < 11) { doc.selection.clear() } else { window.getSelection().removeAllRanges() }
      })
    },
...
(function () {
  function _appendStyle () {
    ...
    var cssText = '.scale{visibility:hidden;cursor:move;position:absolute;left:0;top:0;width:100px;height:50px;background-color:#fff;font-size:0;line-height:0;opacity:.4;filter:Alpha(opacity=40);}' +
...
function ue_callback (url, state) {
  ...
  function scale (img, max, oWidth, oHeight) {
    var width = 0; var height = 0; var percent; var ow = img.width || oWidth; var oh = img.height || oHeight
    if (ow > max || oh > max) {
      if (ow >= oh) {
        if (width = ow - max) {
