...
(function () {
  ...
  function initTabs () {
    ...
    for (var i = 0; i < tabs.length; i++) {
      domUtils.on(tabs[i], 'click', function (e) {
        var j; var bodyId; var target = e.target || e.srcElement
  ...
  function initVideo () {
    ...
    // 编辑视频时初始化相关信息
    (function () {
      var img = editor.selection.getRange().getClosedNode(); var url
  ...
  function createAlignButton (ids) {
    for (var i = 0, ci; ci = ids[i++];) {
      ...
      var nameMaps = { none: lang.default, left: lang.floatLeft, right: lang.floatRight, center: lang.block }
  ...
  function insertUpload () {
    var videoObjs = []
    var uploadDir = editor.getOpt('videoUrlPrefix')
    var width = parseInt($G('upload_width').value, 10) || 420
    var height = parseInt($G('upload_height').value, 10) || 280
    var align = findFocus('upload_alignment', 'name') || 'none'
  ...
  UploadFile.prototype = {
    ...
    initUploader: function () {
      ...
      // WebUploader实例
      var uploader
      var actionUrl = editor.getActionUrl(editor.getOpt('videoActionName'))
      var fileMaxSize = editor.getOpt('videoMaxSize')
