...
*/
function RGBColor (m) {
  this.ok = !1; m.charAt(0) == '#' && (m = m.substr(1, 6)); var m = m.replace(/ /g, ''); var m = m.toLowerCase(); var a = {
    aliceblue: 'f0f8ff',
    antiquewhite: 'faebd7',
    aqua: '00ffff',
    aquamarine: '7fffd4',
    ...
    yellow: 'ffff00',
    yellowgreen: '9acd32'
  }; var c; for (c in a)m == c && (m = a[c]); var d = [{ re: /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/, example: ['rgb(123, 234, 45)', 'rgb(255,234,245)'], process: function (b) { return [parseInt(b[1]), parseInt(b[2]), parseInt(b[3])] } }, {
    re: /^(\w{2})(\w{2})(\w{2})$/,
    example: ['#00ff00', '336699'],
    process: function (b) { return [parseInt(b[1], 16), parseInt(b[2], 16), parseInt(b[3], 16)] }
  }, { re: /^(\w{1})(\w{1})(\w{1})$/, example: ['#fb0', 'f0f'], process: function (b) { return [parseInt(b[1] + b[1], 16), parseInt(b[2] + b[2], 16), parseInt(b[3] + b[3], 16)] } }]; for (c = 0; c < d.length; c++) { var b = d[c].process; var k = d[c].re.exec(m); if (k)channels = b(k), this.r = channels[0], this.g = channels[1], this.b = channels[2], this.ok = !0 } this.r = this.r < 0 || isNaN(this.r) ? 0 : this.r > 255 ? 255 : this.r; this.g = this.g < 0 || isNaN(this.g) ? 0
    : this.g > 255 ? 255 : this.g; this.b = this.b < 0 || isNaN(this.b) ? 0 : this.b > 255 ? 255 : this.b; this.toRGB = function () { return 'rgb(' + this.r + ', ' + this.g + ', ' + this.b + ')' }; this.toHex = function () { var b = this.r.toString(16); var a = this.g.toString(16); var d = this.b.toString(16); b.length == 1 && (b = '0' + b); a.length == 1 && (a = '0' + a); d.length == 1 && (d = '0' + d); return '#' + b + a + d }; this.getHelpXML = function () {
...
(function () {
  function m () {
    ...
    }; a.parseXml = function (a) { if (window.DOMParser) return (new DOMParser()).parseFromString(a, 'text/xml'); else { var a = a.replace(/<!DOCTYPE svg[^>]*>/, ''); var d = new ActiveXObject('Microsoft.XMLDOM'); d.async = 'false'; d.loadXML(a); return d } }; a.Property = function (c, d) {
...
(b /= 100); return b
        ...
        getDefinition: function () { var d = b.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2'); return a.Definitions[d] },
        isUrl: function () {
...
0
        ...
        DPI: function () { return 96 },
        EM: function (b) { var d = 12; var c = new a.Property('fontSize', a.Font.Parse(a.ctx.font).fontSize); c.hasValue() && (d = c.Length.toPixels(b)); return d },
        toPixels: function (d) {
...
g.fontFamily,
        ...
        for (var b = {}, d = a.trim(a.compressSpaces(d || '')).split(' '), k = !1, e = !1, f = !1, g = !1, j = '', h = 0; h < d.length; h++) {
          if (!e && c.Styles.indexOf(d[h]) != -1) { if (d[h] != 'inherit')b.fontStyle = d[h]; e = !0 } else if (!g && c.Variants.indexOf(d[h]) != -1) {
            if (d[h] != 'inherit') {
              b.fontVariant =
d[h]
            }e = g = !0
          } else if (!f && c.Weights.indexOf(d[h]) != -1) { if (d[h] != 'inherit')b.fontWeight = d[h]; e = g = f = !0 } else if (k)d[h] != 'inherit' && (j += d[h]); else { if (d[h] != 'inherit')b.fontSize = d[h].split('/')[0]; e = g = f = k = !0 }
        } if (j != '')b.fontFamily = j; return b
      }
...
a); this.addBezierCurve(b, a, d, d + 1 / 3 * (k - b), c, c + 1 / 3 * (l - a), k, l)
        ...
        var q = [b, a]; var p = [d, c]; var t = [k, l]; var m = [o, n]; this.addPoint(q[0], q[1]); this.addPoint(m[0], m[1]); for (i = 0; i <= 1; i++) {
          b = function (b) { return Math.pow(1 - b, 3) * q[i] + 3 * Math.pow(1 - b, 2) * b * p[i] + 3 * (1 - b) * Math.pow(b, 2) * t[i] + Math.pow(b, 3) * m[i] }, a = 6 * q[i] - 12 * p[i] + 6 * t[i], d = -3 * q[i] + 9 * p[i] - 9 * t[i] + 3 * m[i], c = 3 * p[i] - 3 * q[i], d == 0 ? a != 0 && (a = -c / a, a > 0 && a < 1 && (i == 0 && this.addX(b(a)), i == 1 && this.addY(b(a)))) : (c = Math.pow(a, 2) - 4 * c * d, c < 0 || (k =
(-a + Math.sqrt(c)) / (2 * d), k > 0 && k < 1 && (i == 0 && this.addX(b(k)), i == 1 && this.addY(b(k))), a = (-a - Math.sqrt(c)) / (2 * d), a > 0 && a < 1 && (i == 0 && this.addX(b(a)), i == 1 && this.addY(b(a)))))
        }
...
new a.Property('angle', b)
      ...
      this.applyToPoint = function (b) { for (var a = 0; a < this.transforms.length; a++) this.transforms[a].applyToPoint(b) }; for (var c = a.trim(a.compressSpaces(c)).split(/\s(?=[a-z])/), b = 0; b < c.length; b++) { var k = c[b].split('(')[0]; var e = c[b].split('(')[1].replace(')', ''); this.transforms.push(new this.Type[k](e)) }
    }; a.AspectRatio = function (c, d, b, k, e, f, g, j, h, l) {
      var d = a.compressSpaces(d); var d = d.replace(/^defer\s/, ''); var o = d.split(' ')[0] || 'xMidYMid'; var d = d.split(' ')[1] || 'meet'; var n = b / k; var q = e / f; var p = Math.min(n, q); var m = Math.max(n, q); d == 'meet' && (k *=
...
function () {}; this.clearContext = function () {}; this.renderChildren = function (b) { for (var a = 0; a < this.children.length; a++) this.children[a].render(b) }; this.addChild = function (b, d) { var c = b; d && (c = a.CreateElement(b)); c.parent = this; this.children.push(c) }; if (c != null && c.nodeType == 1) {
        ...
        if (b != null) for (var k in b) this.styles[k] = b[k]; if (this.attribute('class').hasValue()) for (var d = a.compressSpaces(this.attribute('class').value).split(' '), e = 0; e < d.length; e++) { b = a.Styles['.' + d[e]]; if (b != null) for (k in b) this.styles[k] = b[k]; b = a.Styles[c.nodeName + '.' + d[e]]; if (b != null) for (k in b) this.styles[k] = b[k] } if (this.attribute('style').hasValue()) {
          b = this.attribute('style').value.split(';'); for (d = 0; d < b.length; d++) {
      ...
      this.base = a.Element.ElementBase; this.base(c); this.setContext = function (d) {
        if (this.style('fill').Definition.isUrl()) { var b = this.style('fill').Definition.getFillStyle(this); if (b != null)d.fillStyle = b } else if (this.style('fill').hasValue()) {
...
(c = this.style('marker-end').Definition.getDefinition(), c.render(d, b[b.length - 1][0], b[b.length - 1][1]))
      ...
      this.setContext = function (d) {
        d.strokeStyle = 'rgba(0,0,0,0)'; d.lineCap = 'butt'; d.lineJoin = 'miter'; d.miterLimit = 4; this.baseSetContext(d); this.attribute('x').hasValue() && this.attribute('y').hasValue() && d.translate(this.attribute('x').Length.toPixels('x'), this.attribute('y').Length.toPixels('y')); var b = a.ViewPort.width(); var c = a.ViewPort.height(); if (typeof this.root === 'undefined' && this.attribute('width').hasValue() && this.attribute('height').hasValue()) {
          var b = this.attribute('width').Length.toPixels('x'); var c = this.attribute('height').Length.toPixels('y')
          var e = 0; var f = 0; this.attribute('refX').hasValue() && this.attribute('refY').hasValue() && (e = -this.attribute('refX').Length.toPixels('x'), f = -this.attribute('refY').Length.toPixels('y')); d.beginPath(); d.moveTo(e, f); d.lineTo(b, f); d.lineTo(b, c); d.lineTo(e, c); d.closePath(); d.clip()
        }a.ViewPort.SetCurrent(b, c); if (this.attribute('viewBox').hasValue()) {
          var e = a.ToNumberArray(this.attribute('viewBox').value); var f = e[0]; var g = e[1]; var b = e[2]; var c = e[3]; a.AspectRatio(d, this.attribute('preserveAspectRatio').value, a.ViewPort.width(), b, a.ViewPort.height(),
...
new a.Element.PathElementBase(); a.Element.line = function (c) {
      ...
      this.getMarkers = function () { var a = this.getPoints(); var b = a[0].angleTo(a[1]); return [[a[0], b], [a[1], b]] }
      ...
      this.base = a.Element.PathElementBase; this.base(c); this.points = a.CreatePath(this.attribute('points').value); this.path = function (d) {
        var b = new a.BoundingBox(this.points[0].x, this.points[0].y); d != null && (d.beginPath(), d.moveTo(this.points[0].x, this.points[0].y)); for (var c = 1; c < this.points.length; c++) {
...
1] == null && (this.angles[this.angles.length - 1] = this.points[this.points.length - 1].angleTo(d)); this.addMarkerAngle(b, a == null ? null : a.angleTo(b))
        ...
        var b =
this.PathParser; b.reset(); var c = new a.BoundingBox(); for (d != null && d.beginPath(); !b.isEnd();) {
          switch (b.nextCommand(), b.command.toUpperCase()) {
            case 'M':var e = b.getAsCurrentPoint(); b.addMarker(e); c.addPoint(e.x, e.y); d != null && d.moveTo(e.x, e.y); for (b.start = b.current; !b.isCommandOrEnd();)e = b.getAsCurrentPoint(), b.addMarker(e, b.start), c.addPoint(e.x, e.y), d != null && d.lineTo(e.x, e.y); break; case 'L':for (;!b.isCommandOrEnd();) {
              var f = b.current; var e = b.getAsCurrentPoint(); b.addMarker(e, f); c.addPoint(e.x, e.y); d != null &&
d.lineTo(e.x, e.y)
              ...
              var g =
b.current; var f = b.getPoint(); var j = b.getAsControlPoint(); var e = b.getAsCurrentPoint(); b.addMarker(e, j, f); c.addBezierCurve(g.x, g.y, f.x, f.y, j.x, j.y, e.x, e.y); d != null && d.bezierCurveTo(f.x, f.y, j.x, j.y, e.x, e.y)
            } break; case 'S':for (;!b.isCommandOrEnd();)g = b.current, f = b.getReflectedControlPoint(), j = b.getAsControlPoint(), e = b.getAsCurrentPoint(), b.addMarker(e, j, f), c.addBezierCurve(g.x, g.y, f.x, f.y, j.x, j.y, e.x, e.y), d != null && d.bezierCurveTo(f.x, f.y, j.x, j.y, e.x, e.y); break; case 'Q':for (;!b.isCommandOrEnd();) {
            ...
            } break; case 'T':for (;!b.isCommandOrEnd();)g = b.current, j = b.getReflectedControlPoint(), b.control = j, e = b.getAsCurrentPoint(), b.addMarker(e, j, j), c.addQuadraticCurve(g.x, g.y, j.x, j.y, e.x, e.y), d != null && d.quadraticCurveTo(j.x, j.y, e.x, e.y); break; case 'A':for (;!b.isCommandOrEnd();) {
              var g = b.current; var h = b.getScalar(); var l = b.getScalar(); var f = b.getScalar() * (Math.PI / 180); var o = b.getScalar(); var j = b.getScalar()
              var e = b.getAsCurrentPoint(); var n = new a.Point(Math.cos(f) * (g.x - e.x) / 2 + Math.sin(f) * (g.y - e.y) / 2, -Math.sin(f) * (g.x - e.x) / 2 + Math.cos(f) * (g.y - e.y) / 2); var q = Math.pow(n.x, 2) / Math.pow(h, 2) + Math.pow(n.y, 2) / Math.pow(l, 2); q > 1 && (h *= Math.sqrt(q), l *= Math.sqrt(q)); o = (o == j ? -1 : 1) * Math.sqrt((Math.pow(h, 2) * Math.pow(l, 2) - Math.pow(h, 2) * Math.pow(n.y, 2) - Math.pow(l, 2) * Math.pow(n.x, 2)) / (Math.pow(h, 2) * Math.pow(n.y, 2) + Math.pow(l, 2) * Math.pow(n.x, 2))); isNaN(o) && (o = 0); var p = new a.Point(o * h * n.y / l, o * -l * n.x / h); var g = new a.Point((g.x + e.x) / 2 + Math.cos(f) *
p.x - Math.sin(f) * p.y, (g.y + e.y) / 2 + Math.sin(f) * p.x + Math.cos(f) * p.y); var m = function (b, a) { return (b[0] * a[0] + b[1] * a[1]) / (Math.sqrt(Math.pow(b[0], 2) + Math.pow(b[1], 2)) * Math.sqrt(Math.pow(a[0], 2) + Math.pow(a[1], 2))) }; var s = function (b, a) { return (b[0] * a[1] < b[1] * a[0] ? -1 : 1) * Math.acos(m(b, a)) }; var o = s([1, 0], [(n.x - p.x) / h, (n.y - p.y) / l]); var q = [(n.x - p.x) / h, (n.y - p.y) / l]; var p = [(-n.x - p.x) / h, (-n.y - p.y) / l]; var n = s(q, p); if (m(q, p) <= -1)n = Math.PI; m(q, p) >= 1 && (n = 0); j == 0 && n > 0 && (n -= 2 * Math.PI); j == 1 && n < 0 && (n += 2 * Math.PI); q = new a.Point(g.x - h * Math.cos((o + n) /
...
(c = this.attribute('xlink:href').Definition.getDefinition()); for (var e = this.getGradient(d, b), f = 0; f < c.stops.length; f++)e.addColorStop(c.stops[f].offset, c.stops[f].color); if (this.attribute('gradientTransform').hasValue()) {
    ...
    a.Element.GradientBase.prototype = new a.Element.ElementBase(); a.Element.linearGradient = function (c) {
      this.base = a.Element.GradientBase; this.base(c); this.getGradient = function (a, b) {
        var c = b.getBoundingBox(); var e = this.gradientUnits == 'objectBoundingBox' ? c.x() + c.width() * this.attribute('x1').numValue() : this.attribute('x1').Length.toPixels('x'); var f = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('y1').numValue() : this.attribute('y1').Length.toPixels('y'); var g = this.gradientUnits == 'objectBoundingBox'
          ? c.x() + c.width() * this.attribute('x2').numValue() : this.attribute('x2').Length.toPixels('x'); var c = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('y2').numValue() : this.attribute('y2').Length.toPixels('y'); return a.createLinearGradient(e, f, g, c)
      }
    }; a.Element.linearGradient.prototype = new a.Element.GradientBase(); a.Element.radialGradient = function (c) {
      this.base = a.Element.GradientBase; this.base(c); this.getGradient = function (a, b) {
        var c = b.getBoundingBox(); var e = this.gradientUnits == 'objectBoundingBox'
          ? c.x() + c.width() * this.attribute('cx').numValue() : this.attribute('cx').Length.toPixels('x'); var f = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('cy').numValue() : this.attribute('cy').Length.toPixels('y'); var g = e; var j = f; this.attribute('fx').hasValue() && (g = this.gradientUnits == 'objectBoundingBox' ? c.x() + c.width() * this.attribute('fx').numValue() : this.attribute('fx').Length.toPixels('x')); this.attribute('fy').hasValue() && (j = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('fy').numValue()
...
a.Element.animate; this.base(c)
      ...
      this.base = a.Element.ElementBase; this.base(c); this.horizAdvX = this.attribute('horiz-adv-x').numValue(); this.isArabic = this.isRTL = !1; this.missingGlyph = this.fontFace = null; this.glyphs = []; for (c = 0; c < this.children.length; c++) {
        var d = this.children[c]; if (d.type == 'font-face') this.fontFace = d, d.style('font-family').hasValue() && (a.Definitions[d.style('font-family').value] = this); else if (d.type == 'missing-glyph') {
          this.missingGlyph =
d
  ...
  for (var a = this.style('text-anchor').valueOrDefault('start'), c = this.attribute('x').Length.toPixels('x'), d = this.attribute('y').Length.toPixels('y'), j = 0; j < this.children.length; j++) {
    var h = this.children[j]; h.attribute('x').hasValue() ? h.x = h.attribute('x').Length.toPixels('x') : (h.attribute('dx').hasValue() && (c += h.attribute('dx').Length.toPixels('x')), h.x = c); c = h.measureText(b); if (a != 'start' && (j == 0 || h.attribute('x').hasValue())) {
      ...
      this.base = a.Element.RenderedElementBase; this.base(c); this.getGlyph = function (a, b, c) {
        var e = b[c]; var f = null; if (a.isArabic) {
...
1] == ' ') && c < b.length - 2 && b[c + 1] != ' ')g = 'terminal'; c > 0 && b[c - 1] != ' ' && c < b.length - 2 && b[c + 1] != ' ' && (g = 'medial'); if (c > 0 && b[c - 1] != ' ' && (c == b.length - 1 || b[c + 1] == ' '))g = 'initial'; typeof a.glyphs[e] !== 'undefined' && (f = a.glyphs[e][g], f == null && a.glyphs[e].type == 'glyph' && (f = a.glyphs[e]))
        ...
        var b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
          ...
          var e = this.parent.style('font-style').valueOrDefault(a.Font.Parse(a.ctx.font).fontStyle); var f = this.getText(); b.isRTL && (f = f.split('').reverse().join('')); for (var g = a.ToNumberArray(this.parent.attribute('dx').value), j = 0; j < f.length; j++) {
            var h = this.getGlyph(b, f, j); var l = k / b.fontFace.unitsPerEm; c.translate(this.x, this.y); c.scale(l, -l); var o = c.lineWidth; c.lineWidth = c.lineWidth * b.fontFace.unitsPerEm / k; e == 'italic' && c.transform(1, 0, 0.4, 1, 0, 0); h.render(c); e == 'italic' && c.transform(1, 0, -0.4, 1, 0, 0); c.lineWidth = o; c.scale(1 /
l, -1 / l); c.translate(-this.x, -this.y); this.x += k * (h.horizAdvX || b.horizAdvX) / b.fontFace.unitsPerEm; typeof g[j] !== 'undefined' && !isNaN(g[j]) && (this.x += g[j])
        ...
        var b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
          ...
          var k = 0; var e = this.getText(); b.isRTL && (e = e.split('').reverse().join('')); for (var f = a.ToNumberArray(this.parent.attribute('dx').value), g = 0; g < e.length; g++) { var j = this.getGlyph(b, e, g); k += (j.horizAdvX || b.horizAdvX) * c / b.fontFace.unitsPerEm; typeof f[g] !== 'undefined' && !isNaN(f[g]) && (k += f[g]) } return k
    ...
    a.Element.g = function (c) { this.base = a.Element.RenderedElementBase; this.base(c); this.getBoundingBox = function () { for (var c = new a.BoundingBox(), b = 0; b < this.children.length; b++)c.addBoundingBox(this.children[b].getBoundingBox()); return c } }; a.Element.g.prototype = new a.Element.RenderedElementBase(); a.Element.symbol = function (c) {
      this.base = a.Element.RenderedElementBase; this.base(c); this.baseSetContext = this.setContext; this.setContext = function (c) {
        this.baseSetContext(c); if (this.attribute('viewBox').hasValue()) {
          var b =
a.ToNumberArray(this.attribute('viewBox').value); var k = b[0]; var e = b[1]; width = b[2]; height = b[3]; a.AspectRatio(c, this.attribute('preserveAspectRatio').value, this.attribute('width').Length.toPixels('x'), width, this.attribute('height').Length.toPixels('y'), height, k, e); a.ViewPort.SetCurrent(b[2], b[3])
        }
      }
        ...
        if (a.trim(c[d]) != '') {
          for (var b = c[d].split('{'), k = b[0].split(','), b = b[1].split(';'), e = 0; e < k.length; e++) {
            var f = a.trim(k[e]); if (f != '') {
              for (var g = {}, j = 0; j < b.length; j++) { var h = b[j].indexOf(':'); var l = b[j].substr(0, h); var h = b[j].substr(h + 1, b[j].length - h); l != null && h != null && (g[a.trim(l)] = new a.Property(a.trim(l), a.trim(h))) }a.Styles[f] = g; if (f == '@font-face') {
...
this.getDefinition(); b != null && b.path(a)
      ...
      this.base = a.Element.ElementBase; this.base(c); this.apply = function (a, b) {
        var c = this.attribute('x').Length.toPixels('x'); var e = this.attribute('y').Length.toPixels('y'); var f = this.attribute('width').Length.toPixels('x'); var g = this.attribute('height').Length.toPixels('y'); var j = b.attribute('mask').value; b.attribute('mask').value =
...
function (c) { this.base = a.Element.ElementBase; this.base(c); this.apply = function (a) { for (var b = 0; b < this.children.length; b++) this.children[b].path && (this.children[b].path(a), a.clip()) }; this.render = function () {} }; a.Element.clipPath.prototype = new a.Element.ElementBase(); a.Element.filter = function (c) {
      this.base = a.Element.ElementBase; this.base(c); this.apply = function (a, b) {
        var c = b.getBoundingBox(); var e = this.attribute('x').Length.toPixels('x'); var f = this.attribute('y').Length.toPixels('y'); if (e == 0 || f == 0)e = c.x1, f = c.y1; var g =
this.attribute('width').Length.toPixels('x'); var j = this.attribute('height').Length.toPixels('y'); if (g == 0 || j == 0)g = c.width(), j = c.height(); c = b.style('filter').value; b.style('filter').value = ''; var h = 0.2 * g; var l = 0.2 * j; var o = document.createElement('canvas'); o.width = g + 2 * h; o.height = j + 2 * l; var n = o.getContext('2d'); n.translate(-e + h, -f + l); b.render(n); for (var q = 0; q < this.children.length; q++) this.children[q].apply(n, 0, 0, g + 2 * h, j + 2 * l); a.drawImage(o, 0, 0, g + 2 * h, j + 2 * l, e - h, f - l, g + 2 * h, j + 2 * l); b.style('filter', !0).value = c
      }; this.render =
function () {}
          ...
          for (var h = 0; h < f; h++) {
            for (var l = a[j * f * 4 + h * 4 + 3] / 255, o = 0; o < 4; o++) {
              for (var n = d[0] * (l == 0 ? 255 : a[j * f * 4 + h * 4 + o]) * (l == 0 || o == 3 ? 1 : l), q = 1; q < d.length; q++) {
                var p = Math.max(h - q, 0); var m = a[j * f * 4 + p * 4 + 3] / 255; var p = Math.min(h + q, f - 1); var p = a[j * f * 4 + p * 4 + 3] / 255; var s = d[q]; var r; m == 0 ? r = 255 : (r = Math.max(h - q, 0), r = a[j * f * 4 + r * 4 + o]); m = r * (m == 0 || o == 3 ? 1 : m); p == 0 ? r = 255 : (r = Math.min(h + q, f - 1), r = a[j * f * 4 + r * 4 + o]); n +=
s * (m + r * (p == 0 || o == 3 ? 1 : p))
              }c[h * g * 4 + j * 4 + o] = n
            }
          }
        }
      } this.base = a.Element.ElementBase; this.base(c); this.apply = function (a, c, e, f, g) { var e = this.attribute('stdDeviation').numValue(); var c = a.getImageData(0, 0, f, g); var e = Math.max(e, 0.01); var j = Math.ceil(e * 4) + 1; mask = []; for (var h = 0; h < j; h++)mask[h] = Math.exp(-0.5 * (h / e) * (h / e)); e = mask; j = 0; for (h = 1; h < e.length; h++)j += Math.abs(e[h]); j = 2 * j + Math.abs(e[0]); for (h = 0; h < e.length; h++)e[h] /= j; tmp = []; d(c.data, tmp, e, f, g); d(tmp, c.data, e, g, f); a.clearRect(0, 0, f, g); a.putImageData(c, 0, 0) }
...
new a.Element.feGaussianBlur(); a.Element.title = function () {}; a.Element.title.prototype = new a.Element.ElementBase(); a.Element.desc = function () {}; a.Element.desc.prototype = new a.Element.ElementBase(); a.Element.MISSING = function (a) { console.log("ERROR: Element '" + a.nodeName + "' not yet implemented.") }; a.Element.MISSING.prototype = new a.Element.ElementBase(); a.CreateElement = function (c) {
      var d = c.nodeName.replace(/^[^:]+:/, ''); var d = d.replace(/\-/g, ''); var b = null; var b = typeof a.Element[d] !== 'undefined' ? new a.Element[d](c) : new a.Element.MISSING(c)
      ...
      a.init(c); var b = function (a) { for (var b = c.canvas; b;)a.x -= b.offsetLeft, a.y -= b.offsetTop, b = b.offsetParent; window.scrollX && (a.x += window.scrollX); window.scrollY && (a.y += window.scrollY); return a }; if (a.opts.ignoreMouse != !0) {
        c.canvas.onclick = function (c) { c = b(new a.Point(c != null ? c.clientX : event.clientX, c != null ? c.clientY : event.clientY)); a.Mouse.onclick(c.x, c.y) },
        c.canvas.onmousemove = function (c) { c = b(new a.Point(c != null ? c.clientX : event.clientX, c != null ? c.clientY : event.clientY)); a.Mouse.onmousemove(c.x, c.y) }
      } var k = a.CreateElement(d.documentElement); var e = k.root = !0; var f = function () {
        a.ViewPort.Clear(); c.canvas.parentNode && a.ViewPort.SetCurrent(c.canvas.parentNode.clientWidth, c.canvas.parentNode.clientHeight); if (a.opts.ignoreDimensions != !0) {
          if (k.style('width').hasValue())c.canvas.width = k.style('width').Length.toPixels('x'), c.canvas.style.width = c.canvas.width + 'px'; if (k.style('height').hasValue()) {
            c.canvas.height =
k.style('height').Length.toPixels('y'), c.canvas.style.height = c.canvas.height + 'px'
          }
        } var b = c.canvas.clientWidth || c.canvas.width; var d = c.canvas.clientHeight || c.canvas.height; a.ViewPort.SetCurrent(b, d); if (a.opts != null && a.opts.offsetX != null)k.attribute('x', !0).value = a.opts.offsetX; if (a.opts != null && a.opts.offsetY != null)k.attribute('y', !0).value = a.opts.offsetY; if (a.opts != null && a.opts.scaleWidth != null && a.opts.scaleHeight != null) {
          var f = 1; var g = 1; k.attribute('width').hasValue() && (f = k.attribute('width').Length.toPixels('x') /
...
(function (m) {
  var a = m.css; var c = m.CanVGRenderer; var d = m.SVGRenderer; var b = m.extend; var k = m.merge; var e = m.addEvent; var f = m.createElement; var g = m.discardElement; b(c.prototype, d.prototype); b(c.prototype, {
    create: function (a, b, c, d) { this.setContainer(b, c, d); this.configure(a) },
    setContainer: function (a, b, c) {
      var d = a.style; var e = a.parentNode; var g = d.left; var d = d.top; var k = a.offsetWidth; var m = a.offsetHeight; var s = { visibility: 'hidden', position: 'absolute' }; this.init.apply(this, [a, b, c]); this.canvas = f('canvas', { width: k, height: m }, { position: 'relative', left: g, top: d }, a)
    ...
    configure: function (b) {
      var c = this; var d = b.options.tooltip; var f = d.borderWidth; var g = c.ttDiv; var m = d.style; var p = c.ttLine; var t = parseInt(m.padding, 10); var m = k(m, { padding: t + 'px', 'background-color': d.backgroundColor, 'border-style': 'solid', 'border-width': f + 'px', 'border-radius': d.borderRadius + 'px' }); d.shadow && (m = k(m, {
        'box-shadow': '1px 1px 3px gray',
        '-webkit-box-shadow': '1px 1px 3px gray'
      })); a(g, m); a(p, { 'border-left': '1px solid darkgray' }); e(b, 'tooltipRefresh', function (d) {
        var e = b.container; var f = e.offsetLeft; var e = e.offsetTop; var k; g.innerHTML = d.text; k = b.tooltip.getPosition(g.offsetWidth, g.offsetHeight, { plotX: d.x, plotY: d.y }); a(g, { visibility: 'visible', left: k.x + 'px', top: k.y + 'px', 'border-color': d.borderColor }); a(p, { visibility: 'visible', left: f + d.x + 'px', top: e + b.plotTop + 'px', height: b.plotHeight + 'px' }); c.ttTimer !== void 0 && clearTimeout(c.ttTimer); c.ttTimer = setTimeout(function () {
          a(g,
            { visibility: 'hidden' }); a(p, { visibility: 'hidden' })
        }, 3E3)
      })
    },
    destroy: function () { g(this.canvas); this.ttTimer !== void 0 && clearTimeout(this.ttTimer); g(this.ttLine); g(this.ttDiv); g(this.hiddenSvg); return d.prototype.destroy.apply(this) },
    color: function (a, b, c) { a && a.linearGradient && (a = a.stops[a.stops.length - 1][1]); return d.prototype.color.call(this, a, b, c) },
