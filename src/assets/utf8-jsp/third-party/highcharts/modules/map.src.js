...
(function (Highcharts) {
  ...
  var plotOptions = defaultOptions.plotOptions
  ...
  wrap(Chart.prototype, 'render', function (proceed) {
    var chart = this
    var mapNavigation = chart.options.mapNavigation
  ...
  extend(Chart.prototype, {
    renderMapNavigation: function () {
      var chart = this
      var options = this.options.mapNavigation
      var buttons = options.buttons
      var n
      var button
      var buttonOptions
      var outerHandler = function () {
    ...
    mapZoom: function (howMuch, centerXArg, centerYArg) {
      ...
      var centerX = pick(centerXArg, xAxis.min + xRange / 2)
      var newXRange = xRange * howMuch
      var yAxis = chart.yAxis[0]
      var yRange = yAxis.max - yAxis.min
      var centerY = pick(centerYArg, yAxis.min + yRange / 2)
      var newYRange = yRange * howMuch
      var newXMin = centerX - newXRange / 2
      var newYMin = centerY - newYRange / 2
      var animation = pick(chart.options.chart.animation, true)
      var delay
      var newExt = chart.fitToBox({
        x: newXMin,
        y: newYMin,
        width: newXRange,
        height: newYRange
  ...
  var MapAreaPoint = Highcharts.extendClass(Point, {
    ...
    onMouseOut: function () {
      var point = this
      var start = +new Date()
      var normalColor = Color(point.options.color)
      var hoverColor = Color(point.pointAttr.hover.fill)
      var animation = point.series.options.states.normal.animation
      var duration = animation && (animation.duration || 500)
  ...
  seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
    ...
    init: function (chart) {
      var series = this
      var valueDecimals = chart.options.legend.valueDecimals
      var legendItems = []
      var name
      var from
      var to
      ...
      var grad
      var tmpLabel
      var horizontal = chart.options.legend.layout === 'horizontal'
    ...
    drawLegendSymbolGradient: function (legend, item) {
      ...
      var padding = pick(legend.options.padding, 8)
      var positionY
      var positionX
      var gradientSize = this.chart.renderer.fontMetrics(legend.options.itemStyle.fontSize).h
      var horizontal = legend.options.layout === 'horizontal'
      var box1
      var box2
      var box3
      var rectangleLength = pick(legend.options.rectangleLength, 200)
    ...
    getBox: function (paths) {
      ...
      each(paths || this.options.data, function (point) {
        var path = point.path
        var i = path.length
        var even = false // while loop reads from the end
        var pointMaxX = Number.MIN_VALUE
        var pointMinX = Number.MAX_VALUE
        var pointMaxY = Number.MIN_VALUE
        var pointMinY = Number.MAX_VALUE
    ...
    translateColors: function (dataMin, dataMax) {
      ...
      var valueRanges = seriesOptions.valueRanges
      var colorRange = seriesOptions.colorRange
    ...
    drawPoints: function () {
      ...
      each(series.data, function (point) {
        ...
        var maxX = xAxis.toPixels(point._maxX, true)
        var minY = yAxis.toPixels(point._minY, true)
        var maxY = yAxis.toPixels(point._maxY, true)
